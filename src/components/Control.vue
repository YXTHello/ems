...
<template>
	<div>
		<div style="display: inline-block;width: 250px;margin: 10px 0 0 20px;">
			<h1>船队信息</h1>
			<el-table :data="reportList" :row-class-name="tableRowClassName" @row-click="openDetails">
		...
		<div style="display: inline-block;width: 800px;margin: 10px 0 0 20px;vertical-align: top;">
			...
			<div style="font-size: 14px;color: #606266;">
				...
				<el-row style='margin-top: 20px;'>
					...
					<el-col :span="6">
						<el-button type='primary' :disabled="status!='未完成'" @click='startButton'>开始作业</el-button>
						</el-col>
					<el-col :span="6">
					...
					<el-col :span="6">
						<el-button type='success' :disabled="status!='作业中'" @click='completeButton'>结束作业</el-button>
						</el-col>
				</el-row>
				<div class="boat">
					<div class="wrap">
...
<script>
	export default {
		data() {
			...
			//不同流程对应的三个设备
			currentSel(){
				this.dljname = this.flows[this.flowid].dljname;
				this.pdjname = this.flows[this.flowid].pdjname;
				this.zcjname = this.flows[this.flowid].zcjname;
			},
			//开始作业
			startButton(){
		...
		mounted:function(){
			...
			this.$axios.post(url,{
				...
				this.reportList = res.data;
				this.shipname = res.data[0].shipname;
				this.capacity = res.data[0].capacity;
				this.reportuser = res.data[0].reportuser;
				this.planjobtime = res.data[0].planjobtime;
				this.startjobtime = res.data[0].startjobtime;
				this.completetime = res.data[0].completetime;
			})
			// 后端网址
			...
			this.$axios.post(url,{
				...
				this.flows = res.data;
				this.flowid = this.flows[0].flowid;
				this.dljname = this.flows[0].dljname;
				this.pdjname = this.flows[0].pdjname;
				this.zcjname = this.flows[0].zcjname;
			})
		}
	}
</script>
